...
import com.nostra13.universalimageloader.core.decode.RoundDecoderingInfo;
...
final class LoadAndDisplayImageTask implements Runnable {
    ...
    private Bitmap decodeImage(String imageUri) throws IOException {
        ...
        RoundDecoderingInfo decodingInfo = new RoundDecoderingInfo(memoryCacheKey, imageUri, targetSize, imageView.getWidth(), imageView.getHeight(), options.showRoundCorners(), imageView.getScaleType(), viewScaleType, getDownloader(), options);
    ...
    private boolean downloadSizedImage(File targetFile, int maxWidth, int maxHeight) throws IOException {
        ...
        //RoundDecoderingInfo decodingInfo = new RoundDecoderingInfo(memoryCacheKey, uri, targetImageSize, ,ViewScaleType.FIT_INSIDE, getDownloader(), specialOptions);
    ...
    private void fireFailEvent(final FailType failType, final Throwable failCause) {
        if (!Thread.interrupted()) {
            handler.post(new Runnable() {
                ...
                public void run() {
                    ImageView imageView = imageViewRef.get();
                    if (imageView != null && !options.showDefaultBitmap() && options.shouldShowImageOnFail()) {
